...
(facts :image-review :url
       (svc/materialize-image-review-url "uuid_sample" nil) => "http://hostname:9999/image-review/image/uuid_sample?status=ACCEPTED&status=QUESTIONABLE&status=UNREVIEWED"
       (provided
         (consul/find-service consul/create-consul "image-review" "prod") => {:address "hostname", :port 9999}))

(facts :image-review :url
       (svc/materialize-image-review-url "uuid_sample" "REJECTED") => "http://hostname:9999/image-review/image/uuid_sample?status=REJECTED"
       (provided
         (consul/find-service consul/create-consul "image-review" "prod") => {:address "hostname", :port 9999}))
